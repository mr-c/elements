...
func _CombinatorialLibraryDesign_From_Assembly_Standard_4PartRequirements() {
	// e.g. are MoClo types valid?
}

// Conditions to run on startup
func _CombinatorialLibraryDesign_From_Assembly_Standard_4PartSetup(_ctx context.Context, _input *CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput) {
}

// The core process for this protocol, with the steps to be performed
// for every input
func _CombinatorialLibraryDesign_From_Assembly_Standard_4PartSteps(_ctx context.Context, _input *CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput, _output *CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput) {
		...
		for k := range _input.Part1s {
			for l := range _input.Part2s {
				for m := range _input.Part3s {
					key := _input.ProjectName + _input.Vectors[j].Nm + "_" + _input.Part1s[k].Nm + "_" + _input.Part2s[l].Nm + "_" + _input.Part3s[m].Nm
...
func _CombinatorialLibraryDesign_From_Assembly_Standard_4PartAnalysis(_ctx context.Context, _input *CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput, _output *CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput) {
}

// A block of tests to perform to validate that the sample was processed correctly
// Optionally, destructive tests can be performed to validate results on a
// dipstick basis
func _CombinatorialLibraryDesign_From_Assembly_Standard_4PartValidation(_ctx context.Context, _input *CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput, _output *CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput) {
}
func _CombinatorialLibraryDesign_From_Assembly_Standard_4PartRun(_ctx context.Context, input *CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput) *CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput {
	output := &CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput{}
	_CombinatorialLibraryDesign_From_Assembly_Standard_4PartSetup(_ctx, input)
	_CombinatorialLibraryDesign_From_Assembly_Standard_4PartSteps(_ctx, input, output)
	_CombinatorialLibraryDesign_From_Assembly_Standard_4PartAnalysis(_ctx, input, output)
	_CombinatorialLibraryDesign_From_Assembly_Standard_4PartValidation(_ctx, input, output)
	return output
}

func CombinatorialLibraryDesign_From_Assembly_Standard_4PartRunSteps(_ctx context.Context, input *CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput) *CombinatorialLibraryDesign_From_Assembly_Standard_4PartSOutput {
	soutput := &CombinatorialLibraryDesign_From_Assembly_Standard_4PartSOutput{}
	output := _CombinatorialLibraryDesign_From_Assembly_Standard_4PartRun(_ctx, input)
...
func CombinatorialLibraryDesign_From_Assembly_Standard_4PartNew() interface{} {
	return &CombinatorialLibraryDesign_From_Assembly_Standard_4PartElement{
		inject.CheckedRunner{
			RunFunc: func(_ctx context.Context, value inject.Value) (inject.Value, error) {
				input := &CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput{}
			...
			In:  &CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput{},
			Out: &CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput{},
		},
	}
}
...
type CombinatorialLibraryDesign_From_Assembly_Standard_4PartElement struct {
	inject.CheckedRunner
}

type CombinatorialLibraryDesign_From_Assembly_Standard_4PartInput struct {
	...
	StandardLabels           [3]string
	StandardLevel            string
	Vectors                  []wtype.DNASequence
}

type CombinatorialLibraryDesign_From_Assembly_Standard_4PartOutput struct {
...
type CombinatorialLibraryDesign_From_Assembly_Standard_4PartSOutput struct {
	Data struct {
		AssembledSequences    wtype.File
		Assemblies            map[string][]wtype.DNASequence
		EndreportMap          map[string]string
...
func init() {
	if err := addComponent(component.Component{Name: "CombinatorialLibraryDesign_From_Assembly_Standard_4Part",
		Constructor: CombinatorialLibraryDesign_From_Assembly_Standard_4PartNew,
		Desc: component.ComponentDesc{
			Desc: "This protocol is intended to design a combinatorial library of all combinations of a list of Vectors, Promoters,\nRBSs, CDSs and Terminators according to an assembly standard ensuring compatibility with level 1 design.\nLevel 1 adaptor sites (containing the correct restriction site are expected to be included in the promoter and terminator parts.\nThese level 1 sites can be designed such that a series of level 1 parts may be joined together in a second assembly reaction.\nA list of sequencing primers to order will also be returned.\n",
			Path: "src/github.com/antha-lang/elements/an/Data/DNA/TypeIISAssembly_design/CombinatorialDesign/MoClo/Hierarchical/CombinatorialLibraryDesign_From_Assembly_Standard_4Part.an",
